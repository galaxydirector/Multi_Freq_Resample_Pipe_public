...
class Preprocessor(object):
	...
	def __minute_range_helper__(self,configs,stock_data_df,time_range,include_otc=False):
		'''
	...
	def __minute_range__(self,configs,stock_data_df,time_range,include_otc=False):
		...
		'''
		new_df = self.__minute_range_helper__(configs,stock_data_df,time_range,include_otc=False)
	...
	def __datetime_format_process__(self,df):
		...
		if type(df['DATETIME'][0]) == str:
			...
			# pd.to_datetime() is written in C
			# while pd.apply() is a python query
